...
public class SimulatedAnnealing {
    ...
    public Vector<Integer> findBetterAnswer(Vector<Vector<Integer>> vectorOfAllNeighbours) {
        ...
        for (int i = 0; i < vectorOfAllNeighbours.size(); i++) {
            ...
            if (currentDistance < bestCurrentDistance) {
                ...
                int differenceDelta = -(currentDistance - bestCurrentDistance);
                Random rand = new Random();
                double randValue = rand.nextFloat();
                if (Double.compare(randValue, 0) == 0) {
                    randValue = 0.1;
                }
                double x = (Math.exp(Double.valueOf(differenceDelta) / Double.valueOf(temperature)));
                if (Double.compare(randValue, x) < 0) {
                    currentBest = currentSolution;
                }
            }
    ...
    public Vector<Integer> simulatedAnnealingAlgorithm() {
