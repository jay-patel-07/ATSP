...
public class GeneticAlgorithm {
    ...
    public List<DataBean> startingPopulation() {
    ...
    public List<DataBean> selectPopulation(List<DataBean> population) {
    ...
    public static <E> List<E> selectRandoms(List<E> list, int n) {
        Random r = new Random();
    ...
    public DataBean tournamentSelection(List<DataBean> population) {
        List<DataBean> selected = selectRandoms(population, tournamentSize);
        return Collections.min(selected);
    }

    public DataBean doMutation(DataBean salesman) {
        Random random = new Random();
        float doMutation = random.nextFloat();
    ...
    public List<DataBean> createGeneration(List<DataBean> population) {
        ...
        while (currentSize < generationSize) {
            List<DataBean> parents = selectRandoms(population, 2);
            List<DataBean> children = doCrossover(parents);
            children.set(0, doMutation(children.get(0)));
            children.set(1, doMutation(children.get(1)));
    ...
    public List<DataBean> doCrossover(List<DataBean> parents) {
        Random random = new Random();
    ...
    public DataBean optimize() {
        List<DataBean> population = startingPopulation();
        DataBean globalBestGenome = population.get(0);
        for (int i = 0; i < maxIterations; i++) {
            List<DataBean> selected = selectPopulation(population);
